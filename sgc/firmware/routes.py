...
from sgc.firmware.database import (
    init_db, cache_firmware, get_cached_firmware,
    record_installation, get_installation_history,
    get_known_boards, record_board,
)
from sgc.firmware.detection_service import detect_boards, force_bootloader_mode
from sgc.firmware.firmware_repository import (
    get_firmware_cards, get_stable_firmwares,
    get_beta_firmwares, get_legacy_firmwares,
    get_latest_firmwares, refresh_cache,
)
from sgc.firmware.flash_service import (
    download_firmware, verify_checksum, flash_firmware,
)
...
def init_firmware_module(socketio_instance, db_path=None):
    global _socketio
...
def api_download_firmware():
    ...
    if result:
        cache_firmware(
            ...
            data.get("frame", ""), data.get("category", "stable"),
            data.get("url", ""), result.get("checksum_sha256", ""),
            result.get("file_size", 0), result.get("path", ""),
        )
        return jsonify(result)
