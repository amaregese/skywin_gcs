...
class uploader(object):
    ...
    def __verify_v3(self, label, fw):
        ...
        image_crc = fw.crc(len(fw.image))
        self.__send(uploader.GET_CRC + uploader.EOC)
        report_crc = self.__recv_int()
        self.__getSync()
        print("\nCRC check:")
        print("  Expected (padded to fw_maxsize=%u): 0x%x" % (self.fw_maxsize, expect_crc))
        print("  Expected (firmware-only):             0x%x" % image_crc)
        print("  Bootloader GET_CRC:                   0x%x" % report_crc)
        if report_crc != expect_crc:
            print("WARNING: CRC mismatch - expected 0x%x, got 0x%x" % (expect_crc, report_crc))
            print("WARNING: This may be due to fw_maxsize (%u) being larger than actual flash" % self.fw_maxsize)
        else:
            print("CRC OK")
        self.__drawProgressBar(label, 100, 100)
...
def main():
    ...
    parser.add_argument('--port', action="store", help="Comma-separated list of serial port(s)", default=None)
    parser.add_argument('--baud-bootloader', action="store", type=int, default=115200, help="Baud rate for bootloader")
    parser.add_argument('--baud-bootloader-flash', action="store", type=int, default=None, help="Negotiate baudrate for flashing")
    parser.add_argument('--baud-flightstack', action="store", default="57600", help="Comma-separated baud rates for flight stack")
    parser.add_argument('--force', action='store_true', default=False, help='Override board type check')
    ...
    parser.add_argument('--source-system', type=int, action="store", help="Source system for MAVLink", default=255)
    parser.add_argument('--source-component', type=int, action="store", help="Source component for MAVLink", default=0)
    parser.add_argument('--download', action='store_true', default=False, help='download firmware from board')
    parser.add_argument('--identify', action="store_true", help="Do not flash firmware; identify board")
    parser.add_argument('--verify-firmware-is', action="store_true", help="Verify firmware on board matches supplied")
    parser.add_argument('--no-extf', action="store_true", help="Do not attempt external flash operations")
    parser.add_argument('--erase-extflash', type=lambda x: int(x, 0), default=None, help="Erase ext flash")
    parser.add_argument('--force-erase', action="store_true", help="Force full chip erase")
